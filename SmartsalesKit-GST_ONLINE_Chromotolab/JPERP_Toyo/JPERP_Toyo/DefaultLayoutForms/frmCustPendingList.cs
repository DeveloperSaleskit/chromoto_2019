...
namespace Account.DefaultLayout
{
    public partial class frmCustPendingList : WeifenLuo.WinFormsUI.Docking.DockContent
    {
        ...
        private void ArrangeDataGridView()
        {
            try
            {
                dgvContacts.Columns["SalesCode"].DataPropertyName = dtblCustomer.Columns["SalesCode"].ToString();
                //dgvContacts.Columns["ReceiptCode"].DataPropertyName = dtblCustomer.Columns["ReceiptCode"].ToString();
                dgvContacts.Columns["CustomerName"].DataPropertyName = dtblCustomer.Columns["CustomerName"].ToString();
                ...
                dgvContacts.Columns["CustomerCode"].DataPropertyName = dtblCustomer.Columns["CustomerCode"].ToString();
        ...
        private void frmContacts_Load(object sender, EventArgs e)
        {
            ...
            if (CurrentUser.UserID != 1)
            {
                ...
                {
                    if (CurrentUser.PrivilegeStr.IndexOf("#5060#") != -1)
                    {
            ...
            // cmbreports.SelectedIndex = 0;
            //if ((MessageBox.Show(("Do you want to send Autogenerated Mail for Customer pending payment / AMC ?" + ("\r\n" + ("\r\n" + "Are you sure ?"))), "Confirm ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes))
            //{
            //    CurrentCompany.AutoMailPayQuo = "Mail";
            //    mail();
            //}
            TotalPendingAmount();
                ...
                for (int i = 0; i < dgvContacts.Rows.Count; i++)
                {
                    if (Convert.ToInt16(dgvContacts.Rows[i].Cells["DueDays"].Value) != 0)
                    {
                        if (dgvContacts.Rows[i].Cells["DueDate"].Value != DBNull.Value)
                        {
                            ...
                            {
                                SendToMail(dgvContacts.Rows[i].Cells[6].Value.ToString(), dgvContacts.Rows[i].Cells[3].Value.ToString(), dgvContacts.Rows[i].Cells[5].Value.ToString());
                            }
        ...
        private void btnApply_Click(object sender, EventArgs e)
        {
            ...
            frmCustomerPPFilter filterSalesinvoice = new frmCustomerPPFilter(dtblCustomer);
            ...
            if (cmbreports.SelectedIndex == 1)
            {
                try
                {
                    ...
                    if (objList.Exception == null)
                    {
                        ...
                        {
                            //dtblCustomer.TableName = "PORegister";
                            //dtblCustomer.WriteXmlSchema(@"D:\PORegister.xsd");
                            DataView DVReport;
        ...
        private void btnSendMail_Click(object sender, EventArgs e)
        {
            if (dgvContacts.DataSource != null)
            {
                if (dgvContacts.Rows.Count > 0)
                {
                    if ((MessageBox.Show(("Do you want to send Autogenerated Mail for Customer pending payment?" + ("\r\n" + ("\r\n" + "Are you sure ?"))), "Confirm ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes))
                    {
                        //CurrentCompany.AutoMailPayQuo = "Mail";
                        mail();
                    }
