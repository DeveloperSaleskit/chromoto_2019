    ...
    public partial class Splash : Form
    {
        ...
        private void Splash_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ISIValue = ConfigurationManager.AppSettings["ISI"].ToString();

                if (ISIValue == "True")
                {
                    Encryption encry = new Encryption();
                    encry.Decrypt();
                    string constringreg = Common.Encryption.Constr;
                    SqlConnection connection = new SqlConnection(constringreg);
                    connection.Open();
                    string query = "Select RegisterDate,ToDate,IsPaid,LastLoginDate FROM Registration";
                    SqlCommand cmdSelect = new SqlCommand(query, connection);
                    SqlDataAdapter adp = new SqlDataAdapter(cmdSelect);
                    DataTable dtRegistration = new DataTable();
                    adp.Fill(dtRegistration);
                    connection.Close();
                    if ((DateTime.Now.Date > Convert.ToDateTime(dtRegistration.Rows[0]["ToDate"]).Date &&
                        dtRegistration.Rows[0]["IsPaid"].ToString() == "True")
                        || ((DateTime.Now.Date < Convert.ToDateTime(dtRegistration.Rows[0]["ToDate"]).Date) &&
                        (DateTime.Now > Convert.ToDateTime(dtRegistration.Rows[0]["LastLoginDate"])
                        || DateTime.Now.Date >= Convert.ToDateTime(dtRegistration.Rows[0]["LastLoginDate"]).Date)))
                    {
                        connection.Open();
                        string updateQuery = "Update Registration SET LastLoginDate = GetDate()";
                    ...
                    {
                        MessageBox.Show("Please Contact to TechSupport of Saleskit \n for Authentication.", "Validation",
                ...
                {
                    SplashProgress pro = new SplashProgress();
                    pro.ShowDialog();
                    this.Close();

                    //if (checkInstalled("SalesKit"))
                    //{
                    //    //if (ISIKey == null)
                    //    //{
                    ...
                    //else
                    //{
                    //    MessageBox.Show("JPCRM Software is not exist.");
                    //}
                }
            }
            catch (Exception ex)
        ...
        private void loadFromConfig()
        {
            ...
            XmlDocument xmlDoc = new XmlDocument();
            xmlDoc.Load(AppConfigPath + "\\JPCRM.exe.config");
        ...
        public void UpdateKey(string strKey, string newValue)
        {
            ...
            xmlDoc.Save(AppConfigPath + "\\JPCRM.exe.config");
